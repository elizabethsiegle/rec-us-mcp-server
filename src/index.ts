...
export class MyMCP extends McpAgent {
	...
	private getDefaultBookings(): BookingData[] {
		return [
			{ day: 'monday', court: 'DuPont', time: '8:00 AM' },
			{ day: 'wednesday', court: 'McLaren', time: '9:00 AM' },
			{ day: 'friday', court: 'DuPont', time: '10:00 AM' }
		];
	}
	...
	private async completeBookingWithCode(code: string, bookingParams: any) {
		...
		try {
			...
			await page.type('input[id="password"]', recPassword);
			await page.getByText('log in & continue').click();
			await page.waitForTimeout(3000);
	
			// Navigate to court
			await page.getByText(bookingParams.court).click();
			await page.waitForTimeout(2000);
	
			// Navigate to date
			const bookDate = new Date(bookingParams.date);
			const today = new Date();
			today.setFullYear(2025);
			...
			await page.locator(`.react-datepicker__day--0${targetDay < 10 ? '0' : ''}${targetDay}:not(.react-datepicker__day--outside-month)`).first().click();
	
			// Check if time is still available
			await page.waitForSelector('text=/(\\d:)|(No free)/', { timeout: 10000 });
			const times = await page.getByText('Tennis').first().evaluate((el: HTMLElement) => (el.parentElement as HTMLElement).innerText);
	
			if (!times.includes(bookingParams.time)) {
				throw new Error(`${bookingParams.time} no longer available at ${bookingParams.court}`);
			}
	
			// Re-book the time slot
			await page.getByText(bookingParams.time).click();
	
			// Select duration
			await page.locator(`xpath=//label[text()='Duration']/following-sibling::button`).click();
			await page.waitForSelector('text=2 hours');
			await page.getByText(bookingParams.duration || '2 hours').first().click();
			...
			// Enter the SMS code
			this.log(`Entering SMS code: ${code}`);
			...
			for (const selector of inputSelectors) {
				try {
					...
					if (await input.isVisible({ timeout: 3000 })) {
						...
						this.log(`Entered code using: ${selector}`);
						codeEntered = true;
						break;
					}
				} catch (e) {
			...
			if (!codeEntered) {
				throw new Error("Could not find verification input field");
			}
			...
			await page.waitForSelector("text=You're all set!", { timeout: 60000 });
			
			this.log('Booking completed successfully');
	
			// Save successful booking and clean up
			await this.setBookingStatus(bookingParams.dateStr, `${bookingParams.court}: ${bookingParams.time}`);
			await this.setBookingStatus('current_booking_params', '');
...
${bookingParams.court} for ${bookingParams.time} on ${bookingParams.date}
Duration: ${bookingParams.duration}

Your tennis court is now reserved! 🎾`
				}]
			};
	...
	async getCurrentThreads(accessToken: string, email: string) {
	...
	async deleteEmail(accessToken: string, threadId: string, email: string) {
		const response = await fetch(
			`https://gmail.googleapis.com/gmail/v1/users/${email}/threads/${threadId}`,
			{
				method: 'DELETE',
				headers: {
					Authorization: `Bearer ${accessToken}`,
	...
	async getLatestCode(accessToken: string, email: string): Promise<string> {
		...
		while (!emailFound && attempts < maxAttempts) {
			try {
				const threadsData = await this.getCurrentThreads(accessToken, email);
				const threads = (threadsData as any).threads || [];
	
				for (let i = 0; i < threads.length; i++) {
					if (threads[i].snippet && threads[i].snippet.includes("Your REC verification code is: ")) {
						snippet = threads[i].snippet;
						await this.deleteEmail(accessToken, threads[i].id, email);
						emailFound = true;
						this.log('Verification email found and deleted');
						break;
					}
				}
				...
				if (!emailFound) {
					...
					attempts++;
					if (attempts % 10 === 0) {
						this.log(`Still waiting for verification email... (${attempts}s)`);
					}
				}
			} catch (error) {
				this.log(`Error checking for verification email: ${error}`);
		...
		const codeMatch = snippet.match(/Your REC verification code is: (\d+)/);
		if (!codeMatch) {
	...
	async deleteCodeEmail(accessToken: string, email: string) {
		try {
			const threadsData = await this.getCurrentThreads(accessToken, email);
			const threads = (threadsData as any).threads || [];
			...
			this.log(`Error deleting code email: ${error}`);
		}
	}
	...
	private async setBookingStatus(date: string, booking: string): Promise<void> {
		try {
			await (env as any).TENNIS_BOOKINGS?.put(`booking:${date}`, booking);
		} catch (error) {
