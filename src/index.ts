...
The verification page should have an input field for the code.`
					...
					try {
						...
						// 🎯 EXTRACT BOOKING DETAILS AND SAVE TO KV STORAGE
						let bookingCourt = 'Unknown';
						let bookingTime = 'Unknown';
						let bookingDate = new Date().toISOString().split('T')[0];
						
						try {
							const pageContent = await verificationPage.textContent('body', { timeout: 3000 }).catch(() => '');
							console.log('Page content for extraction:', pageContent?.substring(0, 500));
							
							// Try to extract booking details from page content
							const courtMatch = pageContent?.match(/(DuPont|McLaren|Alice Marble)/i);
							if (courtMatch) bookingCourt = courtMatch[1];
							
							const timeMatch = pageContent?.match(/(\d{1,2}:\d{2}\s?[AP]M)/i);
							if (timeMatch) bookingTime = timeMatch[1];
							
							// Try to get date from the current context or URL
							const pageUrl = verificationPage.url();
							console.log('Current page URL:', pageUrl);
							
						} catch (extractError) {
							console.log('Could not extract booking details:', extractError);
						}
						
						// Save the booking to KV storage
						await this.saveBookingToKV(bookingCourt, bookingTime, bookingDate, user.email);
						
						return {
							content: [{
								type: "text",
								text: `🎾 BOOKING COMPLETED & SAVED!

🔐 Completed by: ${user.email}
🏟️ Court: ${bookingCourt}
⏰ Time: ${bookingTime}
📅 Date: ${bookingDate}
✅ SMS code ${code} accepted
✅ "You're all set!" confirmation received
✅ Booking saved to KV storage
